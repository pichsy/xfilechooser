...
ARTIFACT_ID=$(getGradleProperty "PUBLISH_ARTIFACT_ID")
BUILD_DIR="$PROJECT_ROOT/$MODULE_NAME/build/staging-deploy"

echo "开始编译 获取参数：MODULE_NAME=${MODULE_NAME}, ARTIFACT_ID=${ARTIFACT_ID}, BUILD_DIR=${BUILD_DIR}"
...
if [[ -z "$ARTIFACT_ID" ]]; then
  echo "错误：gradle.properties 缺少 PUBLISH_ARTIFACT_ID 配置"
  exit 1
fi

ZIP_NAME="xfilechooser_${PUBLISH_VERSION}.zip"
...
curl -X 'POST' \
  "https://central.sonatype.com/api/v1/publisher/upload?name=$ARTIFACT_ID%3A${PUBLISH_VERSION}&publishingType=AUTOMATIC" \
  -H 'accept: text/plain' \
